    ...
    try {
        ...
        const schemaSQL = `
            ...
            CREATE TABLE discussion_messages (
                id SERIAL PRIMARY KEY,
                discussion_id INTEGER REFERENCES group_discussions(id) ON DELETE CASCADE,
                user_id INTEGER REFERENCES users(id) ON DELETE SET NULL,
                content TEXT NOT NULL
            );
        ...
        const hashedAdminPassword = await bcrypt.hash("testadmin123", BCRYPT_WORK_FACTOR);
        await testClient.query(
            `INSERT INTO users (username, email, password_hash, is_admin)
             VALUES ('testadmin', 'testadmin@example.com', $1, TRUE)
        ...
        await testClient.query(
            `INSERT INTO books (title, cover_image, year_published, synopsis)
             VALUES ('Test Book', 'https://example.com/test.jpg', 2000, 'A test book for testing purposes.')
        ...
        console.log("Test topic added.");

        await testClient.query(`
            INSERT INTO book_authors (book_id, author_id)
            SELECT books.id, authors.id FROM books, authors
            WHERE books.title = 'Test Book' AND authors.name = 'Test Author'
            ON CONFLICT DO NOTHING;
        `);

        await testClient.query(`
            INSERT INTO book_topics (book_id, topic_id)
            SELECT books.id, topics.id FROM books, topics
            WHERE books.title = 'Test Book' AND topics.name = 'Test Topic'
            ON CONFLICT DO NOTHING;
        `);

        console.log("Test relationships added.");
